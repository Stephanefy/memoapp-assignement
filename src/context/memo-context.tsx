...
type MemoContextType = {
  ...
  memos: Memo[] | null;
  setCurrentOpenedCategory: React.Dispatch<
    React.SetStateAction<number | undefined>
  >;
  setCurrentSelectedMemo: React.Dispatch<
    React.SetStateAction<MemoDetails | null>
  >;
...
export const MemoContextProvider = ({ children }: PropsWithChildren) => {
  const [currentOpenedCategory, setCurrentOpenedCategory] = useState<
    number | undefined
  >(undefined);
  const [currentSelectedMemo, setCurrentSelectedMemo] =
    useState<MemoDetails | null>(null);
